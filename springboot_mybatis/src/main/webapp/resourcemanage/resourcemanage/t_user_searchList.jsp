...
<script type="text/javascript">
	...
	function gotoAdd(){
		...
		Usermanager.action="toaddUser.action";
		Usermanager.submit();
	...
	function deletesubmit(obj){
		...
		Usermanager.action = "T_user_detailDelete.action";
		Usermanager.submit();
	}
	...
	function gotoUpdate(){
		...
		if(checkroleIDS()!=false){
			Usermanager.target="_parent";
			Usermanager.action = "T_user_detailUpdate.action";
			Usermanager.submit();
		}
		else{
			return false;
	...
	function checkroleIDS(){                                   
	 ...
	 function showRoleFunction(){
	 	...
	 	if(result!=false){
			self.location= "T_roleAction_findFunctionbyRoleid.action?roleid=" + result ;
...
<tr>
	...
	<td height="15">
		 ...
		 <a href="${basePath}resourcemanage/T_user_detailActionGetPower.action?dealerno=${dealerno}&username=${username}" target="_parent" id="${dealerid }">
		 	<img src='${basePath}/images/freetree/config.gif'  style="cursor: pointer" alt="配置权限" title="配置权限"  height="15" >
		 </a>&nbsp;
		 <a href="${basePath}resourcemanage/T_user_detailDelete.action?dealerno=${dealerno}" target="_parent" >
		 	<img src='${basePath}/images/freetree/delete.gif'  style="cursor: pointer" alt="删除用户" title="删除用户"  height="15" >
		 </a>&nbsp;
		 <a href="${basePath}resourcemanage/T_user_detailUpdate.action?dealerno=${dealerno}" target="_parent" >
...
<tr>
	<td colspan="6" align="center">共 <b><s:property value="paginationSupport.totalPageNo" /></b>页,当前第<b> <s:property value="paginationSupport.currentPageNo " /></b>页
							 <a href='<s:url value="/T_user_detailActionUserList.action">
