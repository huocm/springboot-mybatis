...
function print_arr()
{
	...
	//document.write("------------treedata.length="+n);
	//document.write("------------treedata[0].length="+m);
	for(var i=0; i<n; i++)
	{
		for(j=0; j<m;j++)
		{
			//"  treedata["+i+"]["+j+"]="
			//document.write( treedata["+i+"]["+j+"]=);
...
{											//切换节点的开放/关闭
	...
	//alert("----------menuobj="+menuobj);
	//alert("----------menuobj.style="+menuobj.style);
	//alert("----------menuobj.style.display="+menuobj.style.display);
